...
function [Num_mid,mid,idx]=apcluster(s,p,fea,varargin);
if nargin==0, % display demo
    fprintf('Affinity Propagation (APCLUSTER) sample/demo codenn');
    fprintf('N=100; x=rand(N,2); % Create N, 2-D data pointsn');
    fprintf('M=N*N-N; s=zeros(M,3); % Make ALL N^2-N similaritiesn');
    fprintf('j=1;n');
    fprintf('for i=1:Nn');
    fprintf('  for k=[1:i-1,i+1:N]n');
    fprintf('    s(j,1)=i; s(j,2)=k; s(j,3)=-sum((x(i,:)-x(k,:)).^2);n');
    fprintf('    j=j+1;n');
    fprintf('  end;n');
    fprintf('end;n');
    fprintf('p=median(s(:,3)); % Set preference to median similarityn');
    fprintf('[idx,netsim,dpsim,expref]=apcluster(s,p,''plot'');n');
    fprintf('fprintf(''Number of clusters: %%d\n'',length(unique(idx)));n');
    fprintf('fprintf(''Fitness (net similarity): %%g\n'',netsim);n');
    fprintf('figure; % Make a figures showing the data and the clustersn');
    fprintf('for i=unique(idx)''n');
    fprintf('  ii=find(idx==i); h=plot(x(ii,1),x(ii,2),''o''); hold on;n');
    fprintf('  col=rand(1,3); set(h,''Color'',col,''MarkerFaceColor'',col);n');
    fprintf('  xi1=x(i,1)*ones(size(ii)); xi2=x(i,2)*ones(size(ii)); n');
    fprintf('  line([x(ii,1),xi1]'',[x(ii,2),xi2]'',''Color'',col);n');
    fprintf('end;n');
    fprintf('axis equal tight;nn');
    return;
end;
start = clock;
...
if nargin<2 error('Too few input arguments');
    ...
    maxits=1000; convits=100; lam=0.9; plt=0; details=0; nonoise=1;
    i=1;
    while i<=length(varargin)
        ...
        elseif strcmp(varargin{i},'sparse')
            ...
            fprintf('''sparse'' argument no longer supported; see website for additional softwarenn');
            return;
...
if lam>0.9
    fprintf('n*** Warning: Large damping factor in use. Turn on plottingn');
    fprintf('    to monitor the net similarity. The algorithm willn');
    fprintf('    change decisions slowly, so consider using a larger valuen');
    fprintf('    of convits.nn');
end;
...
if N>3000
    fprintf('n*** Warning: Large memory request. Consider activatingn');
    fprintf('    the sparse version of APCLUSTER.nn');
end;
...
DC=0;DI=0;DP0=0;
diag_AR=diag(A)+diag(R);  %A和R对角线元素求和
I=find(diag_AR>0); K=length(I); % Identify exemplars
[diag_s,ind]=sort(diag_AR,'descend');%把对角线按照降序排列
NN=length(diag_AR); %样本个数

while K<NN/2    %确保样本聚类中心小于总样本的一半
    ...
    for i=1:NN
        DI=DI+S(i,idx(i));    %求类内距离
    end
    DI=DI/NN;
    
    DPX=DC/DI;           %类间距离比上类内距离
    diff=DPX-DP0;        %如果大于零,添加当前中心为新的中心
    DP0=DPX;
    idx0=idx;
...
mid=[];
Num_mid=unique(idx);
for i=1:length(Num_mid)
    mid=[mid;fea(Num_mid(i),:)];
end

for i=1:length(idx)
    if idx(i)==Num_mid(1);
        idx(i)=1;
    elseif idx(i)==Num_mid(2);
        idx(i)=2;
    elseif idx(i)==Num_mid(3);
        idx(i)=3;
    end
end
...
if plt||details
    fprintf('nNumber of exemplars identified: %d  (for %d data points)n',K,N);
    fprintf('Net similarity: %gn',tmpnetsim);
    fprintf('  Similarities of data points to exemplars: %gn',dpsim(end));
    fprintf('  Preferences of selected exemplars: %gn',tmpexpref);
    fprintf('Number of iterations: %dnn',i);
    fprintf('Elapsed time: %g secn',etime(clock,start));
end;
if unconverged
    fprintf('n*** Warning: Algorithm did not converge. Activate plottingn');
    fprintf('    so that you can monitor the net similarity. Considern');
    fprintf('    increasing maxits and convits, and, if oscillations occurn');
    fprintf('    also increasing dampfact.nn');
end;
